...
#define eps 1e-15
#define p64bit double


//////////////////////////////////////////////////////////////////////////////////// PROTOTYPES

///////////////////////////////////////////////////////// FREE MEMORY REQUESTED
void free_memory(p64bit **ZON, p64bit **XDOM, p64bit **YDOM, int **ZMAP, p64bit **QMAP, p64bit **MIU, p64bit **THETA, p64bit **FI, p64bit **W, p64bit **LIST, p64bit **XVALS, p64bit **XVECTS, p64bit **YVALS, p64bit **YVECTS, p64bit **RM, p64bit **PV, p64bit **FM0, p64bit **FM1, p64bit **SM, p64bit **MFLUX, p64bit **MFLOW, p64bit **XFLOW, p64bit **YFLOW);

//////////////////////////////////////////////////////// LOAD PROBLEM FROM FILE
int input_by_txt(int *N, int *nz, p64bit **ZON, int *nxr, p64bit **XDOM, int *nyr, p64bit **YDOM, int **ZMAP, p64bit **QMAP, p64bit *BC, p64bit *tol, const char *filename);

/////////////////////////////////////////////////////////// GENERATE QUADRATURE
int quad(int N, p64bit **MIU, p64bit **THETA, p64bit **CHI, p64bit **W, p64bit **LIST);

////////////////////////////////////////////////////////////////////// SPECTRUM
void myXFunc(int N, p64bit x, p64bit MIU[], p64bit W[], p64bit c0, p64bit *y);

void myXRootFunc(int N, p64bit a, p64bit b, p64bit MIU[], p64bit W[], p64bit c0, p64bit *root);

void myYFunc(int N, p64bit x, p64bit THETA[], p64bit W[], p64bit c0, p64bit *y);

void myYRootFunc(int N, p64bit a, p64bit b, p64bit THETA[], p64bit W[], p64bit c0, p64bit *root);

int spectrum(int N, p64bit MIU[], p64bit THETA[], p64bit LIST[], p64bit W[], int nz, p64bit ZON[], p64bit **xvals, p64bit **xvects, p64bit **yvals, p64bit **yvects);

////////////////////////////////////////////////////////////// MATRIX FUNCTIONS
p64bit* zeros(int M, p64bit **OUTPUT);

p64bit* eye(int M, p64bit **OUTPUT);

p64bit* equal(int M, p64bit Matrix[], p64bit **OUTPUT);

p64bit* vector_neg(int M, p64bit VECTOR[], p64bit **OUTPUT);

p64bit* matrix_neg(int M, p64bit Matrix[], p64bit **OUTPUT);

p64bit* matrix_sum(int M, p64bit Matrix1[], p64bit Matrix2[], p64bit **OUTPUT);

p64bit* matrix_mult1(int M, p64bit Matrix[], p64bit X[], p64bit **OUTPUT);

p64bit* matrix_mult2(int M, p64bit Matrix1[], p64bit Matrix2[], p64bit **OUTPUT);

p64bit* matrix_mult3(int M, p64bit Matrix1[], p64bit Matrix2[], p64bit **OUTPUT);

int inv(int M, p64bit Matrix[], p64bit **OUTPUT);

p64bit* vector_concat(int M, p64bit V1[], p64bit V2[], p64bit **OUTPUT);

p64bit* matrix_concat(int M, p64bit Matrix1[], p64bit Matrix2[], p64bit Matrix3[], p64bit Matrix4[], p64bit **OUTPUT);

////////////////////////////////////////////////////////////// RESPONSE MATRIX
int response_matrix(int N, int nz, p64bit ZON[], int nxr, p64bit XDOM[], int nyr, p64bit YDOM[], int ZMAP[], p64bit QMAP[], p64bit MIU[], p64bit THETA[], p64bit W[], p64bit XVALS[], p64bit XVECTS[], p64bit YVALS[], p64bit YVECTS[], p64bit **RM, p64bit **PV, p64bit **FM0, p64bit **FM1, p64bit **SM);

p64bit* get_RM(int M, int nyr, int nxr, int ry, int rx, p64bit RM[], p64bit **OUTPUT);

p64bit* get_PV(int M, int nyr, int nxr, int ry, int rx, p64bit PV[], p64bit **OUTPUT);

p64bit* get_FM0(int M, int nyr, int nxr, int ry, int rx, p64bit FM0[], p64bit **OUTPUT);

p64bit* get_FM1(int M, int nyr, int nxr, int ry, int rx, p64bit FM1[], p64bit **OUTPUT);

p64bit* get_SM(int M, int nyr, int nxr, int ry, int rx, p64bit SM[], p64bit **OUTPUT);

//////////////////////////////////////////////////////// RM_CN ITERATIVE SCHEME
int rm_lln (int N, int nz, p64bit ZON[], int nxr, p64bit XDOM[], int nyr, p64bit YDOM[], int ZMAP[], p64bit QMAP[], p64bit BC[], p64bit tol, p64bit W[], p64bit RM[], p64bit PV[], p64bit FM0[], p64bit FM1[], p64bit SM[], p64bit **MFLUX, p64bit **MFLOW, p64bit **XFLOW, p64bit **YFLOW, int *ITER, p64bit *cpu_time);

////////////////////////////////////////////////////////// AUXILIARY FUNCTIONS
void json_output(int N, int nxr, p64bit XDOM[], int nyr, p64bit YDOM[], int status, int ITER, p64bit cpu_time, p64bit MFLUX[], p64bit MFLOW[], p64bit XFLOW[], p64bit YFLOW[]);
...
int main(int argc, char *argv[]){
  ...
  p64bit *ZON = NULL;     // Zone entries
  int nxr = 0;                 // Number of regions in X
  p64bit *XDOM = NULL;    // X Region entries
  int nyr = 0;                 // Number of regions in Y
  p64bit *YDOM = NULL;    // Y Region entries
  int *ZMAP = NULL;            // Zone map
  p64bit *QMAP = NULL;    // External source map
  p64bit BC[4] = {0.0};   // Boundary conditions
  p64bit tol = 0.0;       // Tolerance
  const char *filename = NULL; // File name to load

  // QUADRATURE VARIABLES
  p64bit *MIU = NULL;   // Ordinates in X
  p64bit *THETA = NULL; // Ordinates in Y
  p64bit *FI = NULL;    // Ordinates in Z
  p64bit *W = NULL;     // Weight
  p64bit *LIST = NULL;  // Ordinates list

  // SPECTRUM VARIABLES
  p64bit *XVALS = NULL;  // Eigenvalues in X
  p64bit *XVECTS = NULL; // Eigenvectors in X
  p64bit *YVALS = NULL;  // Eigenvalues in Y
  p64bit *YVECTS = NULL; // Eigenvectors in Y

  // RESPONSE MATRIX VARIABLES
  p64bit *RM = NULL;  // Response matrix
  p64bit *PV = NULL;  // Particular vector
  p64bit *FM0 = NULL; // Average flux matrix 0
  p64bit *FM1 = NULL; // Avergae flux matrix 1
  p64bit *SM = NULL;  // Average source vector

  // PRINCIPAL VARIABLES
  p64bit *MFLUX = NULL;  // Escalar flux in the nodes
  p64bit *MFLOW = NULL;  // Angular flux in the nodes
  p64bit *XFLOW = NULL;  // Angular flux at the y edges
  p64bit *YFLOW = NULL;  // Angular flux at the x edges
  int ITER = -1;              // Iterations
  p64bit cpu_time = 0.0; // CPU time
...
int input_by_txt(int *N,                  // Quadrature order
                ...
                p64bit **ZON,        // Zone entries
                int *nxr,                 // Number of regions in X
                p64bit **XDOM,               // X Region entries
                int *nyr,                 // Number of regions in Y
                p64bit **YDOM,               // Y Region entries
                int **ZMAP,               // Zone map
                p64bit **QMAP,       // External source map
                p64bit *BC,          // Boundary conditions
                p64bit *tol,         // Tolerance
  ...
  // ZONE FILLING
  *ZON = malloc(sizeof(p64bit) * (*nz * 2)); if(*ZON == NULL) return 3;
  double st, ss;
	int zone_count = 0;
  for (int z = 0; z < *nz; z++){
      ...
      (*ZON)[z * 2] = (p64bit)st; (*ZON)[z * 2 + 1] = (p64bit)ss;
  ...
  // REGION FILLING IN X
  *XDOM = malloc(sizeof(p64bit) * (*nxr * 2)); 
  if(*XDOM == NULL){
    free(*ZON); return 3;
  }
  int rx_count = 0;
  ...
  for (int xr = 0; xr < *nxr; xr++){
    ...
    (*XDOM)[xr * 2] = (p64bit)len; (*XDOM)[xr * 2 + 1] = (p64bit)nodes;
    rx_count = rx_count + 1;
  }
	if (rx_count != *nxr){
		free(*ZON); free(*XDOM);
  ...
  // REGION FILLING IN Y
  *YDOM = malloc(sizeof(p64bit) * (*nyr * 2));
  if(*YDOM == NULL){
  ...
  for (int yr = 0; yr < *nyr; yr++){
    ...
    (*YDOM)[yr * 2] = (p64bit)len; (*YDOM)[yr * 2 + 1] = (p64bit)nodes;
    ry_count = ry_count + 1;
  }
	if (ry_count != *nyr){
		free(*ZON); free(*XDOM); free(*YDOM);
  ...
  // EXTERNAL SOURCE MAPPING
	*QMAP = malloc(sizeof(p64bit) * (*nyr) * (*nxr)); 
  if(*QMAP == NULL){
...
int quad(int N,                // Quadrature order
         p64bit **MIU,    // Ordinates in X
         p64bit **THETA,  // Ordinates in Y
         p64bit **FI,     // Ordinate list
         p64bit **W,      // Weight
         p64bit **LIST    // Ordinate list
         ){
  ...
  *MIU = malloc(sizeof(p64bit) * M);
  *THETA = malloc(sizeof(p64bit) * M);
  *FI = malloc(sizeof(p64bit) * M);
  *W = malloc(sizeof(p64bit) * M);
  *LIST = malloc(sizeof(p64bit) * N / 2);
  if (*MIU == NULL || *THETA == NULL || *FI == NULL || *W == NULL || *LIST == NULL) return 3;

  // PREFIXED VALUES
  p64bit chi[10] = { 0.0 }, wlist[10] = { 0.0 };
	if (N == 2) {
		chi[0] = 0.5773502692; wlist[0] = 1.0;
	}
	else if (N == 4) {
		chi[0] = 0.3500212; wlist[0] = (p64bit)1/3;
		chi[1] = 0.8688903;
	}
	else if (N == 6) {
...
void myXFunc(int N,              // Quadrature order
             p64bit x,      // X value
             p64bit MIU[],  // Ordinate in X
             p64bit W[],    // Weight
             p64bit c0,     // Scattering Ratio
             p64bit *y)     // Y value
             {
...
int spectrum(int N,                // Quadrature order
            ...
            int nz,               // Number of zones
            p64bit ZON[],    // Zone entries
            p64bit **xvals,  // Eigenvalues in X
            p64bit **xvects, // Eigenvectors in X
            p64bit **yvals,  // Eigenvalues in Y
            p64bit **yvects  // Eigenvectors in Y
            ){
  ...
  // MEMORY ALLOCATION
  *xvals = malloc(sizeof(p64bit) * M * nz);
  *xvects = malloc(sizeof(p64bit) * M * M * nz);
  *yvals = malloc(sizeof(p64bit) * M * nz);
  *yvects = malloc(sizeof(p64bit) * M * M * nz);
  if (*xvals == NULL || *xvects == NULL || *yvals == NULL || *yvects == NULL) return 3;

  // AUXILIARY EIGENVALUES IN X
  p64bit *xvals1 = NULL, *xvals2 = NULL;
  xvals1 = malloc(sizeof(p64bit) * N);
  xvals2 = malloc(sizeof(p64bit) * (M - N));

  // AUXILIARY EIGENVALUES IN Y
  p64bit *yvals1 = NULL, *yvals2 = NULL;
  yvals1 = malloc(sizeof(p64bit) * N);
  yvals2 = malloc(sizeof(p64bit) * (M - N));

  // AUXILIARY EIGENVECTORS IN X
  p64bit *xvects1 = NULL, *xvects2 = NULL;
  xvects1 = malloc(sizeof(p64bit) * M * N);
  xvects2 = malloc(sizeof(p64bit) * M * (M - N));

  // AUXILIARY EIGENVECTORS IN Y
  p64bit *yvects1 = NULL, *yvects2 = NULL;
  yvects1 = malloc(sizeof(p64bit) * M * N);
  yvects2 = malloc(sizeof(p64bit) * M * (M - N));
  ...
  for (int z = 0; z < nz; z++){
    ...
    p64bit st = ZON[z * 2], ss = ZON[z * 2 + 1];
    p64bit c0 = ss / st;
    ...
    if (c0 != 0.0){
      ...
      for(int i = 0; i < N / 2; i++){
        ...
        p64bit chi_i = LIST[i], chi_f, h, a, b, r;
    ...
    // c0 = 0
      ...
      for (int i = 0; i < M; i++){
        p64bit miu = MIU[i], theta = THETA[i];
        (*xvals)[M * z + i] = - miu;
        (*yvals)[M * z + i] = - theta;
      }
    ...
    if (c0 != 0.0){
      ...
      for (int i = 0; i < N; i++){
        for (int m = 0; m < M; m++){
          p64bit miu = MIU[m], theta = THETA[m];
      ...
      for (int i = 0; i < (M - N); i++){
        p64bit val = xvals2[i];
        for (int m = 0; m < M; m++){
          p64bit mw = W[m], mmiu = MIU[m];
          if (val == - mmiu && aux[m] == 0){
            aux[m] = 1;
            for(int n = 0; n < M; n++){
              p64bit nw = W[n], nmiu = MIU[n];
              if (val == - nmiu && aux[n] == 0){
                aux[n] = 1;
                xvects2[M * i + n] = - mw / nw;
                xvects2[M * i + m] = 1.0;
      ...
      for (int i = 0; i < (M - N); i++){
        p64bit val = yvals2[i];
        for (int m = 0; m < M; m++){
          p64bit mw = W[m], mtheta = THETA[m];
          if (val == - mtheta && aux[m] == 0){
            aux[m] = 1;
            for(int n = 0; n < M; n++){
              p64bit nw = W[n], ntheta = THETA[n];
...
p64bit* zeros(int M, p64bit **OUTPUT){
  ...
  if ((*OUTPUT) == NULL){
    *OUTPUT = malloc(sizeof(p64bit) * M * M);
  }
...
p64bit* vector_sum(int M, p64bit Vector1[], p64bit Vector2[], p64bit **OUTPUT){
...
int inv(int M, p64bit Matrix[], p64bit **OUTPUT){
  ...
  p64bit *temp = NULL, *IDEN = NULL, *Matrix2 = NULL;
  temp = malloc(sizeof(p64bit) * M);
  IDEN = malloc(sizeof(p64bit) * M * M);
  Matrix2 = malloc(sizeof(p64bit) * M * M);
...
p64bit* vector_concat(int M, p64bit V1[], p64bit V2[], p64bit **OUTPUT){
  ...
  if ((*OUTPUT) == NULL){
    *OUTPUT = malloc(sizeof(p64bit) * 2 * M);
  }
...
p64bit* matrix_concat(int M, p64bit Matrix1[], p64bit Matrix2[], p64bit Matrix3[], p64bit Matrix4[], p64bit **OUTPUT){
  ...
  if ((*OUTPUT) == NULL){
    *OUTPUT = malloc(sizeof(p64bit) * 4 * M * M);
  }
...
int response_matrix(int N,               // Quadrature order
                     ...
                     p64bit XDOM[],  // X region entries
                     int nyr,             // Number of regions in Y
                     p64bit YDOM[],  // Y regions entries
                     int ZMAP[],          // Zone mapping
                     p64bit QMAP[],  // External source mapping
                     p64bit MIU[],   // Ordinates in X
                     p64bit THETA[], // Ordinates in Y
                     p64bit W[],     // Weight
                     p64bit XVALS[], // Eigenvalues of X
                     p64bit XVECTS[],// Eigenvectors of X 
                     p64bit YVALS[], // Eigenvalues of Y
                     p64bit YVECTS[],// Eigenvectors of Y
                     p64bit **RM,    // Response matrix
                     p64bit **PV,    // Particular vector
                     p64bit **FM0,   // Average flux matrix 0
                     p64bit **FM1,   // Average flux matrix 1
                     p64bit **SM     // Average Source
                     ){
  ...
  // OUTPUT MATRICES
  *RM = malloc(sizeof(p64bit) * 16 * M * M * nyr * nxr);
  *PV = malloc(sizeof(p64bit) * 4 * M * nyr * nxr);
  *FM0 = malloc(sizeof(p64bit) * M * M * nyr * nxr);
  *FM1 = malloc(sizeof(p64bit) * M * M * nyr * nxr);
  *SM = malloc(sizeof(p64bit) * M * nyr * nxr);
  if(*RM == NULL || *PV == NULL || *FM0 == NULL || *FM1 == NULL || *SM == NULL) return 3;

  // AUXILIARY MATRICES
  p64bit *RXIN = NULL, *RXOUT = NULL, *SX = NULL, *FX0 = NULL, *FX1IN = NULL, *FX1OUT = NULL, *AX0 = NULL, *AX1_PLUS = NULL, *AX1_MINUS = NULL;
  RXIN = malloc(sizeof(p64bit) * M * M);
  RXOUT = malloc(sizeof(p64bit) * M * M);
  SX = malloc(sizeof(p64bit) * M); 
  FX0 = malloc(sizeof(p64bit) * M * M);
  FX1IN = malloc(sizeof(p64bit) * M * M);
  FX1OUT = malloc(sizeof(p64bit) * M * M);
  AX0 = malloc(sizeof(p64bit) * M * M);
  AX1_PLUS = malloc(sizeof(p64bit) * M * M);
  AX1_MINUS = malloc(sizeof(p64bit) * M * M);

  p64bit *RYIN = NULL, *RYOUT = NULL, *SY = NULL, *FY0 = NULL, *FY1IN = NULL, *FY1OUT = NULL, *AY0 = NULL, *AY1_PLUS = NULL, *AY1_MINUS = NULL;
  RYIN = malloc(sizeof(p64bit) * M * M);
  RYOUT = malloc(sizeof(p64bit) * M * M);
  SY = malloc(sizeof(p64bit) * M); 
  FY0 = malloc(sizeof(p64bit) * M * M);
  FY1IN = malloc(sizeof(p64bit) * M * M);
  FY1OUT = malloc(sizeof(p64bit) * M * M);
  AY0 = malloc(sizeof(p64bit) * M * M);
  AY1_PLUS = malloc(sizeof(p64bit) * M * M);
  AY1_MINUS = malloc(sizeof(p64bit) * M * M);

  p64bit *SXX = NULL, *XNABLA = NULL, *XF0 = NULL, *XF1 = NULL, *FXX0 = NULL, *FXX1IN = NULL, *FXX1OUT = NULL, *FXX3 = NULL, *FXX4IN = NULL, *FXX4OUT = NULL;
  p64bit *AXX0 = NULL, *AXX1_PLUS = NULL, *AXX1_MINUS = NULL, *AXX3 = NULL, *AXX4_PLUS = NULL, *AXX4_MINUS = NULL;

  SXX = malloc(sizeof(p64bit) * M);
  XNABLA = malloc(sizeof(p64bit) * M * M);
  XF0 = malloc(sizeof(p64bit) * M * M);
  XF1 = malloc(sizeof(p64bit) * M * M);
  FXX0 = malloc(sizeof(p64bit) * M * M);
  FXX1IN = malloc(sizeof(p64bit) * M * M);
  FXX1OUT = malloc(sizeof(p64bit) * M * M);
  FXX3 = malloc(sizeof(p64bit) * M * M);
  FXX4IN = malloc(sizeof(p64bit) * M * M);
  FXX4OUT = malloc(sizeof(p64bit) * M * M);
  AXX0 = malloc(sizeof(p64bit) * M * M);
  AXX1_PLUS = malloc(sizeof(p64bit) * M * M);
  AXX1_MINUS = malloc(sizeof(p64bit) * M * M);
  AXX3 = malloc(sizeof(p64bit) * M * M);
  AXX4_PLUS = malloc(sizeof(p64bit) * M * M);
  AXX4_MINUS = malloc(sizeof(p64bit) * M * M);

  p64bit *SYY = NULL, *YNABLA = NULL, *YF0 = NULL, *YF1 = NULL, *FYY0 = NULL, *FYY1IN = NULL, *FYY1OUT = NULL, *FYY3 = NULL, *FYY4IN = NULL, *FYY4OUT = NULL;
  p64bit *AYY0 = NULL, *AYY1_PLUS = NULL, *AYY1_MINUS = NULL, *AYY3 = NULL, *AYY4_PLUS = NULL, *AYY4_MINUS = NULL;

  SYY = malloc(sizeof(p64bit) * M);
  YNABLA = malloc(sizeof(p64bit) * M * M);
  YF0 = malloc(sizeof(p64bit) * M * M);
  YF1 = malloc(sizeof(p64bit) * M * M);
  FYY0 = malloc(sizeof(p64bit) * M * M);
  FYY1IN = malloc(sizeof(p64bit) * M * M);
  FYY1OUT = malloc(sizeof(p64bit) * M * M);
  FYY3 = malloc(sizeof(p64bit) * M * M);
  FYY4IN = malloc(sizeof(p64bit) * M * M);
  FYY4OUT = malloc(sizeof(p64bit) * M * M);
  AYY0 = malloc(sizeof(p64bit) * M * M);
  AYY1_PLUS = malloc(sizeof(p64bit) * M * M);
  AYY1_MINUS = malloc(sizeof(p64bit) * M * M);
  AYY3 = malloc(sizeof(p64bit) * M * M);
  AYY4_PLUS = malloc(sizeof(p64bit) * M * M);
  AYY4_MINUS = malloc(sizeof(p64bit) * M * M);

  p64bit *LAMBDA_XIN = NULL, *LAMBDA_XOUT = NULL, *GAMA_XIN = NULL, *GAMA_XOUT = NULL;
  p64bit *LAMBDA_YIN = NULL, *LAMBDA_YOUT = NULL, *GAMA_YIN = NULL, *GAMA_YOUT = NULL;
  p64bit *AUX1 = NULL, *AUX2 = NULL, *AUX3 = NULL;
  LAMBDA_XIN = malloc(sizeof(p64bit) * M * M);
  LAMBDA_XOUT = malloc(sizeof(p64bit) * M * M);
  GAMA_XIN = malloc(sizeof(p64bit) * M * M);
  GAMA_XOUT = malloc(sizeof(p64bit) * M * M);
  LAMBDA_YIN = malloc(sizeof(p64bit) * M * M);
  LAMBDA_YOUT = malloc(sizeof(p64bit) * M * M);
  GAMA_YIN = malloc(sizeof(p64bit) * M * M);
  GAMA_YOUT = malloc(sizeof(p64bit) * M * M);
  AUX1 = malloc(sizeof(p64bit) * M * M);
  AUX2 = malloc(sizeof(p64bit) * M * M);
  AUX3 = malloc(sizeof(p64bit) * M * M);

  p64bit *RXIN_INV = NULL, *RYIN_INV = NULL, *M1 = NULL, *M2 = NULL, *M3 = NULL, *M4 = NULL, *MM1 = NULL, *MM2 = NULL, *MM3 = NULL, *MM4 = NULL, *IN = NULL, *OUT = NULL, *OUT_INV = NULL;
  RXIN_INV = malloc(sizeof(p64bit) * M * M);
  RYIN_INV = malloc(sizeof(p64bit) * M * M);
  M1 = malloc(sizeof(p64bit) * M * M);
  M2 = malloc(sizeof(p64bit) * M * M);
  M3 = malloc(sizeof(p64bit) * M * M);
  M4 = malloc(sizeof(p64bit) * M * M);
  MM1 = malloc(sizeof(p64bit) * 4 * M * M);
  MM2 = malloc(sizeof(p64bit) * 4 * M * M);
  MM3 = malloc(sizeof(p64bit) * 4 * M * M);
  MM4 = malloc(sizeof(p64bit) * 4 * M * M);
  IN = malloc(sizeof(p64bit) * 16 * M * M);
  OUT = malloc(sizeof(p64bit) * 16 * M * M);
  OUT_INV = malloc(sizeof(p64bit) * 16 * M * M);

  p64bit *VAUX1 = NULL, *VAUX2 = NULL, *VAUX3 = NULL, *V1 = NULL, *V2 = NULL, *IND1 = NULL, *IND2 = NULL;
  VAUX1 = malloc(sizeof(p64bit) * M);
  VAUX2 = malloc(sizeof(p64bit) * M);
  VAUX3 = malloc(sizeof(p64bit) * M);
  V1 = malloc(sizeof(p64bit) * 2 * M);
  V2 = malloc(sizeof(p64bit) * 2 * M);
  IND1 = malloc(sizeof(p64bit) * 4 * M);
  IND2 = malloc(sizeof(p64bit) * 4 * M);
  ...
  // AUXILIARY VARIABLES
  int ntcx, ntcy, z;
  p64bit lenx, leny, hx, hy, st, ss, c0, Q;
  p64bit m_miu, m_theta, k_miu, k_theta, k_w;

  for (int ry = 0; ry < nyr; ry++){
    for (int rx = 0; rx < nxr; rx++){

      // AUXILIARY VARIABLES
      lenx = XDOM[rx * 2]; ntcx = (int)XDOM[rx * 2 + 1]; hx = (p64bit)(lenx / ntcx);
      leny = YDOM[ry * 2]; ntcy = (int)YDOM[ry * 2 + 1]; hy = (p64bit)(leny / ntcy);
...
int rm_lln (int N,               // Quadrature order
           ...
           p64bit BC[],    // Boundary conditions
           p64bit tol,     // Tolerance
           p64bit W[],     // Quadrature weight
           p64bit RM[],    // Response matrix
           p64bit PV[],    // Source vector
           p64bit FM0[],   // Average flux matrix 0
           p64bit FM1[],   // Average flux matrix 1
           p64bit SM[],    // Average source vector
           p64bit **MFLUX, // Escalar flux in the nodes
           p64bit **MFLOW, // Angular flux in the nodes
           p64bit **XFLOW, // Angular flux at the y edges
           p64bit **YFLOW, // Angular flux at the x edges
           int *ITER,            // Iteration
           p64bit *cpu_time // CPU time
           ){
  ...
  int M = N * (N + 2) / 2;
  *MFLUX = malloc(sizeof(p64bit) * (ntc_y * ntc_x));
	*MFLOW = malloc(sizeof(p64bit) * (ntc_y * ntc_x) * M);
	*XFLOW = malloc(sizeof(p64bit) * (ntc_y * (ntc_x + 1)) * M);
	*YFLOW = malloc(sizeof(p64bit) * ((ntc_y + 1) * ntc_x) * M);
  if (*MFLUX == NULL || *MFLOW == NULL || *XFLOW == NULL || *YFLOW == NULL) return 3;

  p64bit *XXFLOW = NULL, *YYFLOW = NULL;
  XXFLOW = malloc(sizeof(p64bit) * (ntc_y * (ntc_x + 1)) * M);
  YYFLOW = malloc(sizeof(p64bit) * (ntc_y * (ntc_x + 1)) * M);
  ...
  int j_b, j_f, i_b, i_f;
  int nc_y, nc_x;
  p64bit *IN = NULL, *OUT = NULL; 
  p64bit *RESP_MATRIX = NULL, *PART_VECTOR = NULL, *V_AUX = NULL;
  IN = malloc(sizeof(p64bit) * 4 * M);
  OUT = malloc(sizeof(p64bit) * 4 * M);
  RESP_MATRIX = malloc(sizeof(p64bit) * 16 * M * M);
  PART_VECTOR = malloc(sizeof(p64bit) * 4 * M);
  V_AUX = malloc(sizeof(p64bit) * 4 * M);
  p64bit *X_VECTOR = NULL, *Y_VECTOR = NULL, *M_VECTOR = NULL, *V_AUX2 = NULL;
  p64bit *FM_0 = NULL, *FM_1 = NULL, *S_VECTOR = NULL;
  X_VECTOR = malloc(sizeof(p64bit) * M);
  Y_VECTOR = malloc(sizeof(p64bit) * M);
  M_VECTOR = malloc(sizeof(p64bit) * M);
  V_AUX2 = malloc(sizeof(p64bit) * M);
  FM_0 = malloc(sizeof(p64bit) * M * M);
  FM_1 = malloc(sizeof(p64bit) * M * M);
  S_VECTOR = malloc(sizeof(p64bit) * M);
  p64bit m_sum, w, flux0, flux;
  ...
  *cpu_time = (p64bit)(end - start) / CLOCKS_PER_SEC;
...
void json_output(int N,               // Quadrature order
                 ...
                 int ITER,            // Iterations
                 p64bit cpu_time,// CPU time
	               p64bit MFLUX[], // Scalar flux in the nodes
                 p64bit MFLOW[], // Angular flux in the nodes
                 p64bit XFLOW[], // Angular flux at the y edges
                 p64bit YFLOW[]  // Angular flux at the x edges
					 ){
